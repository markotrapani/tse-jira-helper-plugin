...
| `DOC-6506-bug-go-redis-endpoint-type.md` | Bug | DOC project / docs.redis.io / wrong default value | **DOC project bugs** — incorrect technical content (wrong default, wrong type, wrong example) discovered via internal Slack or customer ticket. **DOC schema is sparse**: no Severity / Component / Environment / Product / Seen by Customer / Found By / Issue source / RCA template. Don't apply RED-style customfield set. |
| `RDSC-4706-bug-rdi-debezium-oracle-rac.md` | Bug | RDSC / RDI / Debezium / Oracle RAC | **RDSC bugs use dedicated `Steps to Reproduce` / `Expected Result` / `Actual Result` custom fields** (NOT description H2 sections). Description body becomes a customer Q&A transcript. No 6-section RCA template, no Severity, no Found By. Labels: `Support-Attention`, `rca_related`. Cloners link when same defect affects multiple customers. |
| `MOD-12739-bug-amr-inconsistent-count.md` | Bug | MOD / AMR / Azure / inconsistent counts | **AMR-routed MOD Bug** with `AMR` + `Azure-Integration` labels, populated `ICM ID/s` field, `Affected Organizations = Azure`, `Found by = Community` (not Prod/Customer). **RCA template sections 0-1 fully populated at file time** — required for Azure customer-facing automation. MOD-specific `BugScore` + `Is Bug Description Set` Boolean fields. |
| `RED-186235-bug-low-severity-ui-cosmetic.md` | Bug | Severity 3-Low / cosmetic UI / Boolean impact fields | **Sev-3 RED Bugs** for cosmetic UI inconsistencies. `Data loss: No` + `Data unavailable: No` Boolean pair confirms zero operational impact. Uses `SM Component` sub-component field. Minimal flat-narrative description (no H2 sections), embedded screenshot, CLI verification block, one-line `ASK:` suffix. |
| `RCA-563-rca-cluster-reshard-stuck.md` | RCA | Cluster-incident shape RCA | **Cluster-incident RCA** with `#prod__<date>_<cluster>-<account>_<symptom>` title (NOT `<Customer> - RCA <date>`). `Reporter: Incident` (automation), `causes` CINC+PRB links, emoji-prefixed `## Summary` body, dedicated `Cluster ID` field. **Initial Root Cause is 1-2 sentences** here vs multi-paragraph in customer-RCA shape. Long technical narrative goes in description body's `🕵️‍♂️ What happened (in detail):` line. |
